...
def closer_city(lat, lon, city1, city2):
    ...
    dist1 = distance(temp_city, city1)
    dist2 = distance(temp_city, city2)
    if dist1 < dist2 :
...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    #return [create_row(rows)] * columns
...
def put_piece(board, max_rows, column, player):
    ...
    #My solution
    """
    row = max_rows - 1 #初始状态，若此column中未有任何Piece，则新投入的piece应落入第max_rows -1 行
    for i in range(max_rows): #判断此column种是否已有piece
        if get_piece(board, i, column) != '-':
            row = i - 1
            break
    new_board = board
    if row != -1:       # 在此column未满的情况下，更新new_board.否则保留new_board= board
        new_board[row] = replace_elem(board[row], column, player)
    return (row, new_board)
    """
    #Alternative solution: Does not cross abstraction barrier
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for i in range(max_rows - num_connect + 1):
        consecutive = True
        for column in range(i, i + num_connect):
    ...
    #Alternative Solution: use "adjacent" variable (+1 or reset to 0) to track whether elements are consecutive
    adjacent = 0
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for i in range(max_cols - num_connect + 1):
        consecutive = True
        for row in range(i, i + num_connect):
